...
class CheckYourAnswersHelper(userAnswers: UserAnswers)(implicit messages: Messages) {
	...
	def employmentDetails: Option[AnswerRow] = userAnswers.employmentDetails map {
		x =>
			AnswerRow("employmentDetails.checkYourAnswersLabel",
	...
	def enterPayeReference: Option[AnswerRow] = userAnswers.enterPayeReference map {
		x =>
			AnswerRow("enterPayeReference.checkYourAnswersLabel",
	...
	def selectBenefits: Option[AnswerRow] = userAnswers.selectBenefits map {
		...
		x =>
			AnswerRow(keyPrefix + "checkYourAnswersLabel", x.map {
	...
	def anyOtherCompanyBenefits: Option[AnswerRow] = userAnswers.anyOtherCompanyBenefits map {
		x =>
			AnswerRow(
				"anyOtherCompanyBenefits.checkYourAnswersLabel",
				if (x) "site.yes" else "site.no",
				true,
				Some(routes.AnyOtherCompanyBenefitsController.onPageLoad(CheckMode).url)
			)
	}
	...
	def anyTaxableIncome: Option[AnswerRow] = userAnswers.anyTaxableIncome map {
		x =>
			AnswerRow("anyTaxableIncome.checkYourAnswersLabel",
				if (x) "site.yes" else "site.no",
				true,
				Some(routes.AnyTaxableIncomeController.onPageLoad(CheckMode).url)
			)
	}
	...
	def selectTaxableIncome: Option[AnswerRow] = userAnswers.selectTaxableIncome map {
		...
		x =>
			AnswerRow(keyPrefix + "checkYourAnswersLabel",
				x.map {
				...
				false,
				Some(routes.SelectTaxableIncomeController.onPageLoad(CheckMode).url)
			)
	}
	...
	def howMuchRentalIncome: Option[AnswerRow] = userAnswers.howMuchRentalIncome map {
		x =>
			AnswerRow(
				"howMuchRentalIncome.checkYourAnswersLabel",
				s"£$x",
				false,
				Some(routes.HowMuchRentalIncomeController.onPageLoad(CheckMode).url)
			)
	}
	...
	def howMuchBankInterest: Option[AnswerRow] = userAnswers.howMuchBankInterest map {
		x =>
			AnswerRow(
				"howMuchBankInterest.checkYourAnswersLabel",
				s"£$x",
				false,
				Some(routes.HowMuchBankInterestController.onPageLoad(CheckMode).url)
			)
	}
	...
	def howMuchInvestmentOrDividend: Option[AnswerRow] = userAnswers.howMuchInvestmentOrDividend map {
		x =>
			AnswerRow(
				"howMuchInvestmentOrDividend.checkYourAnswersLabel",
				s"£$x",
				false,
				Some(routes.HowMuchInvestmentOrDividendController.onPageLoad(CheckMode).url)
			)
	}
	...
	def howMuchForeignIncome: Option[AnswerRow] = userAnswers.howMuchForeignIncome map {
		x =>
			AnswerRow(
				"howMuchForeignIncome.checkYourAnswersLabel",
				s"£$x",
				false,
				Some(routes.HowMuchForeignIncomeController.onPageLoad(CheckMode).url)
			)
	}
	...
	def anyOtherTaxableIncome: Option[AnswerRow] = userAnswers.anyOtherTaxableIncome map {
		x =>
			AnswerRow("anyOtherTaxableIncome.checkYourAnswersLabel",
				if (x) "site.yes" else "site.no",
				true,
				Some(routes.AnyOtherTaxableIncomeController.onPageLoad(CheckMode).url)
			)
	}
	...
	def whereToSendPayment: Option[AnswerRow] = userAnswers.whereToSendPayment map {
		x =>
			AnswerRow(
				"whereToSendPayment.checkYourAnswersLabel",
				s"whereToSendPayment.$x",
				true,
				Some(routes.WhereToSendPaymentController.onPageLoad(CheckMode).url)
			)
	}
	...
	def paymentAddressCorrect: Option[AnswerRow] = userAnswers.paymentAddressCorrect map {
		x =>
			AnswerRow(
				"paymentAddressCorrect.checkYourAnswersLabel",
				if (x) "site.yes" else "site.no",
				true,
	...
	def nomineeFullName: Option[AnswerRow] = userAnswers.nomineeFullName map {
		x =>
			AnswerRow(
				"nomineeFullName.checkYourAnswersLabel",
				s"$x",
				false,
				Some(routes.NomineeFullNameController.onPageLoad(CheckMode).url)
			)
	}
	...
	def anyAgentRef: Option[AnswerRow] = userAnswers.anyAgentRef map {
		x =>
			AnswerRow(
				"anyAgentRefOption.checkYourAnswersLabel",
				x match {
	...
	def agentReferenceNumber: Option[AnswerRow] = userAnswers.anyAgentRef match {
		case Some(AnyAgentRef.Yes(number)) =>
			Some(AnswerRow(
				"anyAgentRef.checkYourAnswersLabel",
				s"$number",
				false,
				Some(routes.AnyAgentRefController.onPageLoad(CheckMode).url)
			))
	...
	def isPaymentAddressInTheUK: Option[AnswerRow] = userAnswers.isPaymentAddressInTheUK map {
		x =>
			AnswerRow(
				"isPaymentAddressInTheUK.checkYourAnswersLabel",
				if (x) "site.yes" else "site.no",
				true,
	...
	def paymentUKAddress: Option[AnswerRow] = userAnswers.paymentUKAddress map {
		x =>
			AnswerRow(
				"paymentUKAddress.checkYourAnswersLabel",
				UkAddress.asString(
					UkAddress(
						x.addressLine1,
						x.addressLine2,
						...
						x.postcode)),
				false,
				Some(routes.PaymentUKAddressController.onPageLoad(CheckMode).url)
			)
	}
	...
	def paymentInternationalAddress: Option[AnswerRow] = userAnswers.paymentInternationalAddress map {
		x =>
			AnswerRow(
				"paymentInternationalAddress.checkYourAnswersLabel",
				InternationalAddress.asString(
					InternationalAddress(
						...
						x.country)),
				false,
				Some(routes.PaymentInternationalAddressController.onPageLoad(CheckMode).url)
			)
	}
	...
	def anyTelephoneNumber: Option[AnswerRow] = userAnswers.anyTelephoneNumber map {
		x =>
			AnswerRow(
				"telephoneNumberOption.checkYourAnswersLabel",
				x match {
