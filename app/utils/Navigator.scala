...
class Navigator @Inject()() {
  ...
  private val routeMapWithIndex: PartialFunction[Identifier, UserAnswers => Call] = {
    case OtherTaxableIncomeId(index) => otherTaxableIncome(NormalMode)
  }

  private val editRouteMapWithIndex: PartialFunction[Identifier, UserAnswers => Call] = {
    case OtherTaxableIncomeId(index) => otherTaxableIncome(CheckMode)
  }
...
//    OtherTaxableIncomeId -> otherTaxableIncome(CheckMode),
  ...
  def otherTaxableIncome(mode: Mode)(userAnswers: UserAnswers): Call =
    if (mode == NormalMode) routes.AnyOtherTaxableIncomeController.onPageLoad(mode) else routes.CheckYourAnswersController.onPageLoad()
