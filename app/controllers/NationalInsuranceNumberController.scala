  ...
  def onSubmit(mode: Mode) = (authenticate andThen getData andThen requireData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        (value) =>
          dataCacheConnector.save[String](request.externalId, NationalInsuranceNumberId.toString, value).map(cacheMap =>
            Redirect(navigator.nextPage(NationalInsuranceNumberId, mode)(new UserAnswers(cacheMap))))
      )
  }
}
