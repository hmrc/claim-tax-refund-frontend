...
class EnterPayeReferenceController @Inject()(
  ...
  def onPageLoad(mode: Mode) = (authenticate andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.enterPayeReference match {
        case None => form
        case Some(value) => form.fill(value)
      }
      Ok(enterPayeReference(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(enterPayeReference(appConfig, formWithErrors, mode))),
        (value) =>
