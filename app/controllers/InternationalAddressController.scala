...
import models.{Mode, InternationalAddress}
import utils.{Navigator, UserAnswers}
import views.html.internationalAddress

import scala.concurrent.Future

class InternationalAddressController @Inject()(appConfig: FrontendAppConfig,
      ...
      form.bindFromRequest().fold(
        ...
        (value) =>
          dataCacheConnector.save[InternationalAddress](request.externalId, InternationalAddressId.toString, value).map(cacheMap =>
            Redirect(navigator.nextPage(InternationalAddressId, mode)(new UserAnswers(cacheMap))))
      )
  }
}
