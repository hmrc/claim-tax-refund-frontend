...
class HowMuchJobseekersAllowanceController @Inject()(
                                                      ...
                                                      formBuilder: HowMuchJobseekersAllowanceForm) extends FrontendController with I18nSupport {
  ...
  def onPageLoad(mode: Mode) = (authenticate andThen getData andThen requireData) {
    implicit request =>
      ...
      Ok(howMuchJobseekersAllowance(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(howMuchJobseekersAllowance(appConfig, formWithErrors, mode))),
        (value) =>
