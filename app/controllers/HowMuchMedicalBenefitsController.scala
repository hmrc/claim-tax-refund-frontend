...
class HowMuchMedicalBenefitsController @Inject()(
                                        ...
                                        formBuilder: HowMuchMedicalBenefitsForm) extends FrontendController with I18nSupport {
  ...
  def onPageLoad(mode: Mode) = (authenticate andThen getData andThen requireData) {
    implicit request =>
      ...
      Ok(howMuchMedicalBenefits(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(howMuchMedicalBenefits(appConfig, formWithErrors, mode))),
        (value) =>
