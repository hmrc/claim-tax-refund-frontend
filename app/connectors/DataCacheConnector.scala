...
class DataCacheConnectorImpl @Inject()(val sessionRepository: SessionRepository, val cascadeUpsert: CascadeUpsert)(implicit executionContext: ExecutionContext) extends DataCacheConnector {
  ...
  def removeAll(cacheId: String): Future[Boolean] = {
        val newCacheMap = new CacheMap(cacheId, Map())
        sessionRepository().upsert(newCacheMap)
  }
