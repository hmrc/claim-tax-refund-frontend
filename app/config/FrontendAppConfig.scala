...
class FrontendAppConfig @Inject() (override val configuration: Configuration) extends AppName with BaseUrl {
  ...
  lazy val whitelistedIps: Seq[String] = Some(
    new String(Base64.getDecoder.decode(configuration.getString("microservice.services.whitelist.ips").getOrElse("")),
    "UTF-8")
  ).map(_.split(",")).getOrElse(Array.empty).toSeq

  lazy val whitelistExcluded = Some(new String(Base64.getDecoder.decode(configuration.getString("microservice.services.whitelist.excluded").getOrElse("")), "UTF-8"))
    .map(_.split(",")).getOrElse(Array.empty).toSeq
  ...
  lazy val appUrl = baseUrl("claim-tax-refund")
