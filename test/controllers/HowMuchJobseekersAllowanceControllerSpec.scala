...
class HowMuchJobseekersAllowanceControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new HowMuchJobseekersAllowanceController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction,
      dataRetrievalAction, new DataRequiredActionImpl, new HowMuchJobseekersAllowanceForm(frontendAppConfig))

  val testAnswer = "9,999.99"
  val form = new HowMuchJobseekersAllowanceForm(frontendAppConfig)()

  def viewAsString(form: Form[_] = form) = howMuchJobseekersAllowance(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "HowMuchJobseekersAllowance Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(HowMuchJobseekersAllowanceId.toString -> JsString(testAnswer))
