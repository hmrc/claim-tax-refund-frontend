...
class SelectTaxableIncomeControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new SelectTaxableIncomeController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction,
      dataRetrievalAction, new DataRequiredActionImpl, new SelectTaxableIncomeForm(frontendAppConfig))

  val testAnswer = "answer"
  val form = new SelectTaxableIncomeForm(frontendAppConfig)()

  def viewAsString(form: Form[_] = form) = selectTaxableIncome(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "SelectTaxableIncome Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(SelectTaxableIncomeId.toString -> JsString(testAnswer))
