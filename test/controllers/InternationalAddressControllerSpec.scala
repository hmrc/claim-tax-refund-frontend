...
class InternationalAddressControllerSpec extends ControllerSpecBase {
  ...
  "InternationalAddress Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(InternationalAddressId.toString -> Json.toJson(InternationalAddress("value 1", "value 2")))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad(NormalMode)(fakeRequest)

      contentAsString(result) mustBe viewAsString(InternationalAddressForm().fill(InternationalAddress("value 1", "value 2")))
    }
