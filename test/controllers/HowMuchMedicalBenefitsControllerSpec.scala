...
class HowMuchMedicalBenefitsControllerSpec extends ControllerSpecBase with GuiceOneAppPerSuite {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new HowMuchMedicalBenefitsController(messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction(authConnector, frontendAppConfig),
      dataRetrievalAction, new DataRequiredActionImpl(messagesControllerComponents), howMuchMedicalBenefits, messagesControllerComponents, new HowMuchMedicalBenefitsForm(frontendAppConfig))
