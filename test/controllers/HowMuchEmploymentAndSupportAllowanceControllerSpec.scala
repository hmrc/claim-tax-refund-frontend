...
class HowMuchEmploymentAndSupportAllowanceControllerSpec extends ControllerSpecBase {
  ...
  val form = new HowMuchEmploymentAndSupportAllowanceForm(frontendAppConfig)()

  def viewAsString(form: Form[_] = form) = howMuchEmploymentAndSupportAllowance(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "HowMuchEmploymentAndSupportAllowance Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(HowMuchEmploymentAndSupportAllowanceId.toString -> JsString(testAnswer))
