...
class HowMuchEmploymentAndSupportAllowanceControllerSpec extends ControllerSpecBase with GuiceOneAppPerSuite {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new HowMuchEmploymentAndSupportAllowanceController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      FakeAuthAction(authConnector, frontendAppConfig), dataRetrievalAction, new DataRequiredActionImpl(messagesControllerComponents), howMuchEmploymentAndSupportAllowance, messagesControllerComponents, new HowMuchEmploymentAndSupportAllowanceForm(frontendAppConfig), formPartialRetriever, templateRenderer)
  ...
  "HowMuchEmploymentAndSupportAllowance Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      when(mockUserAnswers.howMuchEmploymentAndSupportAllowance).thenReturn(Some(testAnswer))
