...
class HowMuchIncapacityBenefitControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new HowMuchIncapacityBenefitController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction,
      dataRetrievalAction, new DataRequiredActionImpl, new HowMuchIncapacityBenefitForm(frontendAppConfig))

  val testAnswer = "9,999.99"
  val form = new HowMuchIncapacityBenefitForm(frontendAppConfig)()

  def viewAsString(form: Form[_] = form) = howMuchIncapacityBenefit(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "HowMuchIncapacityBenefit Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(HowMuchIncapacityBenefitId.toString -> JsString(testAnswer))
