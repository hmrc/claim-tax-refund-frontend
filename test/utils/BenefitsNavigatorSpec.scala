    ...
    "in normal mode" when {
      ...
      "Navigating from SelectBenefits" must {
        ...
        "go to OtherBenefitsName when other-taxable-benefit checkbox is the first answer selected" in {
          ...
          navigator.nextPage(SelectBenefitsId, NormalMode)(answers) mustBe routes.OtherBenefitsNameController.onPageLoad(NormalMode, 0)
        }
      }
      ...
      "Navigating from HowMuchBereavementAllowance" must {
        ...
        "go to OtherBenefitsName if this option was selected on SelectBenefits" in {
          ...
          navigator.nextPage(HowMuchBereavementAllowanceId, NormalMode)(answers) mustBe routes.OtherBenefitsNameController.onPageLoad(NormalMode, 0)
        }
      ...
      "go to HowMuchOtherBenefit from OtherBenefitsName" in {
        val answers = mock[UserAnswers]
        navigator.nextPageWithIndex(OtherBenefitsNameId(0), NormalMode)(answers) mustBe routes.HowMuchOtherBenefitController.onPageLoad(NormalMode, 0)
      }
      ...
      "go to OtherBenefitsName from AnyOtherBenefits when answer is yes" in {
        ...
        when(answers.otherBenefitsName) thenReturn Some(Seq("test benefit"))
    ...
    "when in checkMode" must {
      ...
      "Navigating from OtherBenefitsName" must {
        "go to CheckYourAnswersController when name and amount stored" in {
          ...
          navigator.nextPage(OtherBenefitsNameId(0), CheckMode)(answers) mustBe routes.CheckYourAnswersController.onPageLoad()
        }
        ...
        "go to CheckYourAnswers when no amount stored and in CheckMode" in {
          val answers = MockUserAnswers.benefitsUserAnswers
          when(answers.howMuchOtherBenefit) thenReturn None

          navigator.nextPageWithIndex(OtherBenefitsNameId(0), CheckMode)(answers) mustBe routes.CheckYourAnswersController.onPageLoad()
        }
      }
