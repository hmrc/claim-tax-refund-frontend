...
class UniqueTaxpayerReferenceViewSpec extends StringViewBehaviours {

  val messageKeyPrefix = "uniqueTaxpayerReference"

  def createView = () => uniqueTaxpayerReference(frontendAppConfig, UniqueTaxpayerReferenceForm(), NormalMode)(fakeRequest, messages)

  def createViewUsingForm = (form: Form[String]) => uniqueTaxpayerReference(frontendAppConfig, form, NormalMode)(fakeRequest, messages)

  val form = UniqueTaxpayerReferenceForm()
