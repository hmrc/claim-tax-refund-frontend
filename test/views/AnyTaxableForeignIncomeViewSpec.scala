...
class AnyTaxableForeignIncomeViewSpec(implicit messages: Messages) extends QuestionViewBehaviours[AnyTaxPaid] {

  val messageKeyPrefix = "anyTaxableForeignIncome"
  ...
  def createView = () => anyTaxableForeignIncome(frontendAppConfig, form, NormalMode, taxYear)(fakeRequest, messages)

  def createViewUsingForm = (form: Form[_]) => anyTaxableForeignIncome(frontendAppConfig, form, NormalMode, taxYear)(fakeRequest, messages)
  ...
  "AnyTaxableForeignIncome view" must {
    ...
    yesNoPage(createViewUsingForm, messageKeyPrefix, routes.AnyTaxableForeignIncomeController.onSubmit(NormalMode).url)
