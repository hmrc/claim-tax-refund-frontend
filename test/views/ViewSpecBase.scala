...
trait ViewSpecBase extends SpecBase {
  ...
  def assertDynamicPageTitleEqualsMessage(doc: Document, expectedMessageKey: String, dynamicHeading: String, args: Any*) = {
    val headers = doc.getElementsByTag("h1")
    headers.size mustBe 1
    headers.first.text.replaceAll("\u00a0", " ") mustBe (messages(expectedMessageKey, args: _*).replaceAll("&nbsp;", " ") +" "+ dynamicHeading)
  }
